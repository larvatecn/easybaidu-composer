...
use EasyBaidu;
use EasyBaiduComposer\Http\DelegationResponse;
...
class Hydrate
{
    ...
    protected function createsApplication()
    {
        ...
        if ($application === EasyBaidu\OpenPlatform\Authorizer\OfficialAccount\Application::class) {
            return $this->createsOpenPlatformApplication('officialAccount');
        }

        if ($application === EasyBaidu\OpenPlatform\Authorizer\MiniProgram\Application::class) {
    ...
    protected function createsOpenPlatformApplication($type)
    {
        ...
        return EasyBaidu\Factory::openPlatform($this->buildConfig($config))->$type($authorizerAppId, $config['refresh_token']);
    }
